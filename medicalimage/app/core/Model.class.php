...
class Model
{
     ...
     * @return mixed
     */
    public static function loadOne1($column, $value) {
        // create our sql for the lookup
        $sQL = 'SELECT * '
                . 'FROM `%s` '
                . 'WHERE `%s` = :value and status="active" '
                . 'LIMIT 1';
        $sQL = sprintf($sQL, self::getTableName(), $column);
    ...
    public static function loadMore($column, $value) {
        // create our sql for the lookup
        $sQL = 'SELECT * '
                . 'FROM `%s` '
                . 'WHERE `%s` = :value ';
    ...
    public static function lastRM($clauseStr = '', $replacements = array()) {
        // create our sql for the lookup
        $sQL = 'SELECT MAX(`rekam_medis`) AS max '
                . 'FROM `%s` ';
        if (strlen($clauseStr) > 0) {
            $sQL .= 'WHERE ' . $clauseStr;
        }
