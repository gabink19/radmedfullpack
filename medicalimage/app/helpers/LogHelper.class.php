...
class LogHelper
{
    ...
    static function globalErrorHandler($errno, $errstr, $errfile, $errline) {
        ...
        /*try {
            ...
            $ErrorLog->file =  $errfile;
            $ErrorLog->line = $errline;
            $ErrorLog->error_type = self::friendlyErrorType($errno) ;
            if ($ErrorLog->message == 'Trying to access array offset on value of type bool' && $ErrorLog->file=='/var/www/html/medicalimage/app/core/Database.class.php'&& $ErrorLog->line=='204' && $ErrorLog->error_type=='E_NOTICE') {
            }else if($ErrorLog->message == 'Undefined index: _t' && $ErrorLog->file=='/var/www/html/medicalimage/app/core/Framework.class.php'&& $ErrorLog->line=='121' && $ErrorLog->error_type=='E_NOTICE') {
            }else{
                $ErrorLog->save();
            }
        } catch (\Exception $e) {
